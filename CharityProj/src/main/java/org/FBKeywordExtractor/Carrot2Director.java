...
public class Carrot2Director {
	...
	public ArrayList<Keyword> DoCluster(List<FqlPage> pages) {
		...
		for (int i = 0; i < clustersByTopic.size(); i++) {
			...
			if (clustersByTopic.get(i).getDocuments().size() > SUPPORT_RATIO
					* noPages
		...
		for (Cluster cluster : clusters) {
			result.add(new Keyword("Liked Pages", cluster.getLabel(), cluster
					.getDocuments().size(), cluster.getScore()));
		}
